...
<div class="container">
    ...
    <form method="POST" action="{{ route('admin.reservations.update', $booking->id) }}">
        ...
        <div class="mb-3">
            ...
            <input type="time" name="time_start" value="{{ old('time_start', $booking->time_start) }}" class="form-control">
        </div>

        <div class="mb-3">
            <label>Eindtijd</label>
            <input type="time" name="time_end" value="{{ old('time_end', $booking->time_end) }}" class="form-control">
        </div>
        ...
        <div class="mb-3 mt-4">
            ...
            <select name="invoice_status" class="form-select">
                <option value="onbetaald" {{ $booking->invoice->status === 'onbetaald' ? 'selected' : '' }}>Onbetaald</option>
                <option value="betaald" {{ $booking->invoice->status === 'betaald' ? 'selected' : '' }}>Betaald</option>
            </select>
        </div>
        @endif
